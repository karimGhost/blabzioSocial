...
if (recipientFCMToken && postLike) {
  ...
  return (
     ...
     <>
      <Head>
        <title>{post.title || "Blabzio Feed"}</title>
        <meta property="og:title" content={post.title || "Blabzio Post"} />
               <meta property="og:description" content={post.content?.slice(0, 100) || "View this post on Blabzio!"} />

        <meta property="og:image" content={post?.mediaUrl  || "/android-chrome-192x192.png"} />
        <meta property="og:url" content={`https://blabzio.vercel.app/feed/${post.id}`} />
